...
//                    myTacker.clear();
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        if (class_flag==-2) {
            ...
            Core.gemm(theta, x_val, 1, new Mat(), 0.0, yMat, 0);
            // ======== END YOUR CODE HERE ========
            ...
            // ======== START YOUR CODE HERE ========
            float temp = -1; // initialize max_value to the first element in y_hat array

            // iterate over the remaining elements in y_hat array to find the maximum element
            for (int i = 0; i < 10; i++) {
                if (y_hat[i] > temp) {
                    temp = y_hat[i];
                    number_detected = i; // assign the index of the maximum element to number_detected
                }
            }

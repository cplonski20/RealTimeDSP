...
while True:
    ...
    if tracking_flag == -1:
        ...
        # Your code starts here
        fps = cv2.getTickCount()
        flag,roi = tracker.update(frame)
        fps = cv2.getTickFrequency()/(cv2.getTickCount() -fps)
        if(flag == False):
            cv2.putText(frame,"Tracking failure occured!", (40,40),cv2.FONT_HERSHEY_SIMPLEX,1, color=(0,0,255))
        else:
            cv2.putText(frame,f"FPS@: {fps}", (60,60),cv2.FONT_HERSHEY_SIMPLEX,1, color=(0,0,255))
